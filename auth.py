...
class TokenBlacklist:
    """Redis-based token blacklist with in-memory fallback.

    Uses Redis for distributed token blacklist across multiple service instances.
    Falls back to in-memory if Redis is unavailable.

    SOLID Principles:
    - Single Responsibility: Only handles token blacklisting
    - Open/Closed: Can extend with different storage backends
    - Liskov Substitution: Compatible with previous API
    """

    _fallback_blacklist = set()  # Fallback if Redis unavailable
    _redis_client = None
    ...
    def _get_redis(cls):
        ...
        if cls._redis_client is None:
            try:
                ...
                logger.warning("redis_client module not found, using in-memory fallback")
        return cls._redis_client

    @classmethod
    def add_token(cls, token: str, expires_in_seconds: int = None) -> None:
        """Add a token to the blacklist.

        Args:
            token: JWT token to blacklist
            expires_in_seconds: Time until token naturally expires (for Redis TTL)
        """
        ...
        if redis and redis.is_available:
            ...
            success = redis.blacklist_token(token, expires_in_seconds)
            if success:
                logger.debug(f"Token blacklisted in Redis (TTL: {expires_in_seconds}s)")
                return
            else:
                logger.warning("Redis blacklist failed, using in-memory fallback")

        # Fallback to in-memory
        cls._fallback_blacklist.add(token)
    ...
    def is_blacklisted(cls, token: str) -> bool:
        ...
        # Check Redis first
        if redis and redis.is_available:
            return redis.is_token_blacklisted(token)

        # Fallback to in-memory
        return token in cls._fallback_blacklist
    ...
    def clear(cls) -> None:
        ...
        # Clear Redis
        if redis and redis.is_available:
            redis.clear_blacklist()

        # Clear in-memory fallback
        cls._fallback_blacklist.clear()
